...
class G:  # Glyphs
    ...
    OBJECTS = frozenset(
        {nh.GLYPH_OBJ_OFF + i for i in range(nh.NUM_OBJECTS) if ord(nh.objclass(i).oc_class) != nh.ROCK_CLASS}
    )
    BOULDER = frozenset(
        {nh.GLYPH_OBJ_OFF + i for i in range(nh.NUM_OBJECTS) if ord(nh.objclass(i).oc_class) == nh.ROCK_CLASS}
    )
    ...
    TRAPS = frozenset(
        {
            SS.S_arrow_trap,
            SS.S_dart_trap,
            SS.S_falling_rock_trap,
            SS.S_squeaky_board,
            SS.S_bear_trap,
            SS.S_land_mine,
            SS.S_rolling_boulder_trap,
            SS.S_sleeping_gas_trap,
            SS.S_rust_trap,
            SS.S_fire_trap,
            SS.S_pit,
            SS.S_spiked_pit,
            SS.S_hole,
            SS.S_trap_door,
            SS.S_teleportation_trap,
            SS.S_level_teleporter,
            SS.S_magic_portal,
            SS.S_web,
            SS.S_statue_trap,
            SS.S_magic_trap,
            SS.S_anti_magic_trap,
            SS.S_polymorph_trap,
        }
    )
...
G.INV_DICT = {i: [k for k, v in G.DICT.items() if i in v] for i in set.union(*map(set, G.DICT.values()))}
